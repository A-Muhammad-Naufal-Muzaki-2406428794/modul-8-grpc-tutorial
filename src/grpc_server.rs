...
impl ChatService for MyChatService {
    ...
    async fn chat(
        ...
        let (tx, rx) = mpsc::channel(10); // Channel untuk mengirim balasan

        tokio::spawn(async move {
            // Loop untuk terus mendengarkan pesan dari klien
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let chat_service = MyChatService::default(); // Tambahkan baris ini
    
    Server::builder()
        .add_service(PaymentServiceServer::new(payment_service))
        .add_service(TransactionServiceServer::new(transaction_service))
        .add_service(ChatServiceServer::new(chat_service)) // Tambahkan baris ini
        .serve(addr)
        .await?;
