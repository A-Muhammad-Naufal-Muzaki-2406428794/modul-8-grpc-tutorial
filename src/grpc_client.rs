...
use services::{
    ...
    chat_service_client::ChatServiceClient, ChatMessage // Tambahkan ini
};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // --- Bagian Unary (Payment) yang sudah ada sebelumnya ---
    ...
    // --- Bagian Baru: Server Streaming (Transaction History) ---
    ...
    // Loop untuk menerima setiap pesan dari stream sampai habis
    while let Some(transaction) = stream.message().await? {
        println!("Transaction: {:?}", transaction);
    }

    // --- Bagian Baru: Bi-Directional Streaming (Chat) ---
    
    // Inisialisasi koneksi untuk Chat Service
    let channel = Channel::from_static("http://[::1]:50051").connect().await?;
    let mut client = ChatServiceClient::new(channel);

    // Channel untuk mengirim input terminal ke background task gRPC
    let (tx, rx): (Sender<ChatMessage>, Receiver<ChatMessage>) = mpsc::channel(32);

    // Background task untuk membaca input dari keyboard
